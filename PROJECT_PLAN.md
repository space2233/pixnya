...
> 状态：`1.1.0` 功能范围已实现，进入 `1.1.0` 验证阶段
> 日期：2026-08-11
> 当前源码版本：`1.1.0`；最新公开稳定版：`1.0.0`；生产签名升级基线：`0.29.0`
...
3. **低安全直连模式**：通过内置 Pixiv IP 白名单绕过异常解析，关闭上游 SNI 和证书验证；默认禁用且只能由用户主动选择。1.1 起风险弹窗默认不显示，可从设置恢复。Android 网页登录与 token 交换都可使用该传输，设置页必须披露密码、验证码和令牌泄露风险。

核心浏览版本的成功标准不是功能数量最大化，而是证明以下闭环可靠成立：

- 标准模式安全完成官方网页登录；非标准 Android 登录必须由用户主动选择并在设置页保留中间人风险说明；
...
- 低安全直连不得自动启用、保存为默认模式或作为 ECH 的静默回退；1.1 起重复风险 UI 默认抑制，用户可在设置中恢复，Rust 调用仍必须携带由主动模式选择产生的明确确认状态。
- 低安全直连只用于明确白名单内的 Pixiv OAuth/API/媒体主机和用户主动选择的 Android 登录桥；设置页必须明确提示 token 同样可能泄露，并允许恢复重复提醒。
...
- Android 低安全登录桥必须由策略明确标记并取得确认状态；1.1 默认抑制重复弹窗，但 UI 仍为每次 Rust 调用传入由主动模式选择产生的显式确认。兼容模式的 token 请求也必须被策略标为低安全；
...
| 低安全直连 | 内置 Pixiv IP；关闭 SNI/证书验证；1.1 默认不弹警告，可在设置恢复 | 同左 | Android 使用一次性指纹锁定的低安全 TLS 桥；桌面保留端到端 WebView TLS | 不自动启用，不作为 ECH 回退 |
...
- 默认关闭且只能主动选择；设置页持续说明中间人攻击、数据篡改和 access token 泄露风险。1.1 默认不弹重复警告，并能随时从设置恢复。
- Rust 客户端连接固定候选 IP，关闭 SNI 与服务器证书验证；UI 必须如实显示这一点，不得称为“安全直连”。
- Android 登录 WebView 与 OAuth token 交换只有在取得由主动模式选择产生的明确确认状态后才可使用低安全传输；设置页分别显示密码、验证码和 token 泄露风险，用户可随时恢复弹窗。
...
当前源码已完成 `1.1.0` 功能范围，下一步只做完整验证、真实测试账号低频验收和发布说明。未排期事项统一记录在[备选功能计划](docs/OPTIONAL_FEATURES_PLAN.md)。
