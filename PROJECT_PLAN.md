...
> 状态：`1.2.0` 首次连接选择、设置精简与发布验证已完成
> 日期：2026-08-11
> 当前源码版本：`1.2.0`；最新公开稳定版：`1.2.0`；生产签名升级基线：`0.29.0`
...
- 三种连接模式只改变到白名单 Pixiv 主机的网络传输方式，不绕过登录、付费、年龄、地区或账号权限控制。低安全路径可能遭受中间人攻击；1.2 起应用内不再显示风险标签、确认弹窗或重复警告，风险边界改由项目安全与隐私文档持续记录。
...
3. **低安全直连模式**：通过内置 Pixiv IP 白名单绕过异常解析，关闭上游 SNI 和证书验证；不预选且只能由用户主动选择。Android 网页登录与 token 交换都可使用该传输；1.2 起界面不显示风险标签、确认弹窗或重复警告。

核心浏览版本的成功标准不是功能数量最大化，而是证明以下闭环可靠成立：

- 标准模式安全完成官方网页登录；非标准 Android 登录必须由用户主动选择，且不得静默回退；
- 三种连接配置的能力和限制可被检测、展示和诊断；
- 稳定浏览作品、作者、推荐、排行榜和搜索结果；
- 正确加载、缓存和下载普通图片，基础播放 Ugoira；
- 不泄露 token、Cookie、密码、访问历史或未脱敏日志。

### 1.2 当前实现进度（2026-08-11）
...
- [x] 历史版本曾提供低安全连接警告与恢复入口；1.2 起改为首次设置中主动选择统一连接模式，兼容模式可持久化且不显示重复警告。
- [x] `1.1.0`：只读站内通知支持分组展开与 `next_url` 分页；通知正文纯文本渲染并限制链接边界；插画/小说评论支持仅本人删除、失败恢复，以及官方贴图目录和文字/单贴图组合发表。
- [x] `1.1.0`：在线或已下载 Ugoira 可作为可取消后台任务导出 GIF、APNG 和 WebM，保留原始帧时序并检查空间、帧数、分辨率与内存预算；取消和失败清理不完整文件。
- [x] `1.1.0`：官方 access-block 和用户/标签 mute 列表与增删操作已接入，账号写入要求确认和串行提交，并与仅本机隐藏/报告明确隔离。
- [x] `1.1.0`：离线资料库支持批量移动、批量标签和批量删除，保存复合筛选器，并按 Pixiv 资源 ID、内容类型和文件 SHA-256 只报告重复内容。
- 三条登录路线的真实账号活体测试已暂停并移入[备选功能计划](docs/OPTIONAL_FEATURES_PLAN.md)。
- 当前功能范围和逐版本验收边界见[功能路线图](docs/FEATURE_ROADMAP.md)；更新分发继续由[自动检查更新与自动更新](docs/AUTO_UPDATE_PLAN.md)维护。
...
- 低安全直连不得自动启用、预选或作为 ECH 的静默回退；用户明确选择后可以持久化，Rust 调用仍必须携带由主动模式选择产生的明确确认状态。
- 低安全直连只用于明确白名单内的 Pixiv OAuth/API/媒体主机和用户主动选择的 Android 登录桥；1.2 起应用内不显示风险标签、确认弹窗或重复提醒。
...
- Android 低安全登录桥必须由策略明确标记并取得确认状态；1.2 界面不显示重复警告，但 UI 仍为每次 Rust 调用传入由主动模式选择产生的显式确认。兼容模式的 token 请求也必须被策略标为低安全；
...
| ECH | DoH `HTTPS` 记录 + TLS 1.3 + ECH，必须确认接受 | 支持 ECH 时使用；否则显示失败 | 桌面由 WebView 内核协商；Android 登录明确失败关闭 | OAuth/数据不静默降级；Android 登录不把普通 TLS 冒充 ECH |
| 低安全直连 | 内置 Pixiv IP；关闭 SNI/证书验证；1.2 仅由用户主动选择统一连接模式 | 同左 | Android 使用一次性指纹锁定的低安全 TLS 桥；桌面保留端到端 WebView TLS | 不自动启用，不作为 ECH 回退 |
...
- 不预选且只能主动选择；选择可持久化，但不得因其他路线失败自动启用。安全与隐私文档持续记录中间人攻击、数据篡改和 access token 泄露风险。
- Rust 客户端连接固定候选 IP，关闭 SNI 与服务器证书验证；UI 必须如实显示这一点，不得称为“安全直连”。
- Android 登录 WebView 与 OAuth token 交换只有在取得由主动模式选择产生的明确确认状态后才可使用低安全传输；1.2 起界面不显示风险标签、确认弹窗或重复警告。
- ECH 或标准模式失败时不得自动回退到该模式。
- 当前固定 IP 只作为可更新的本地版本配置，不接受远端响应扩大域名白名单。

### 7.4 登录 WebView 的能力边界

普通 CONNECT 代理可以替 WebView 选择 DNS 结果和目标 IP，却无法隐藏 WebView ClientHello 的 SNI。Android 仅为用户主动选择的兼容登录采用低安全 TLS 终止桥，规则为：

- 标准登录保持系统 TLS；Windows/Linux 兼容登录保留端到端 WebView TLS；
- Android 系统 WebView 无法证明 ECH `Accepted`，因此 ECH 登录明确失败关闭且不得映射到普通 TLS 或兼容桥；
- Android 兼容登录取得由主动模式选择产生的确认状态后，使用一次性证书、固定 IP、无上游 SNI/证书验证的本地桥；
...
核心客户端当前已具备登录、会话安全存储、三种连接路线、主要浏览与账号操作、只读通知、评论管理、图片/Ugoira 导出、官方屏蔽/静音、小说、下载队列、高级离线资料库、本机历史和脱敏诊断。

每个 `1.x` 功能版本的完成标准：

- [x] 新增功能具有 Rust/TypeScript 类型边界、Tauri 命令和前端错误恢复。
...
`1.2.0` 已完成首次连接选择、统一连接模式、设置界面精简和大容量缓存档位。后续只处理已确认的缺陷与[备选功能计划](docs/OPTIONAL_FEATURES_PLAN.md)中重新排期的事项。
...
- [1.1.0 功能范围](docs/FEATURE_ROADMAP.md)
