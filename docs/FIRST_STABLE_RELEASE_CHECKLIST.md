...
> 状态：首个稳定版与后续 `1.1.0`–`1.5.1` 已发布；当前维护范围是 `1.6.5`
> 当前升级基线：公开稳定版 `1.5.1`
...
- [x] 发布前阻断运行时 npm 低危以上告警、全部 npm 高危以上告警、RustSec advisory，并用 OSV 扫描 Android ARM APK 的共同 runtime 锁图；runtime 零例外。构建工具图使用 2026-09-12 扫描确认的 88 条精确、限 scope、限版本、限期的临时 OSV 基线（2026-09-02 的 84 条仍于 2026-10-02 到期；新增 4 条 `io.netty:netty-handler` 构建期告警：`GHSA-c4c3-7fpv-j4q5` Critical 于 2026-09-26 到期，`GHSA-fccg-mwvh-qqg4` Moderate 于 2026-10-12 到期）。此前 Bouncy Castle Critical 已通过升级到 `1.80.2` 消除。新增、变化或到期即失败，原始报告随每个 Release 归档。
...
- [x] 从固定提交 `302e117e234aff16658c1a6f82c707c1697cd40e` 触发 `1.5.1` 签名 Draft；GitHub Actions run `33730303676` 完成五平台构建、签名与 10 附件汇总。该 Release 曾作为 Pre-release 公开；官方 Publish 只接受 Draft，无法再复验，维护者随后直接将 `v1.5.1` 标为 latest stable。本轮不做人机活体验收。
...
- [ ] `1.6.5`：Windows x64 从公开 `1.5.1` 覆盖升级活体验收。尚未执行，不得预先填写 PASS、设备名或安装包 SHA-256。
- [ ] `1.6.5`：Android ARM64 从公开 `1.5.1` 覆盖升级，并覆盖标准、ECH、兼容三种连接模式。尚未执行。`1.6.3` 上维护者确认过新功能可用，不自动转记为本轮通过。
- [ ] Linux x64、Windows ARM64、Android ARM32：`1.6.5` 仅要求 CI 签名构建。
...
`1.5.1` 已是公开 latest stable；固定候选 `302e117e234aff16658c1a6f82c707c1697cd40e`、五平台签名、10 个附件已由 Draft 工作流 `33730303676` 生成。该 Release 曾作为 Pre-release 公开，官方 Publish 工作流只接受 Draft，无法再复验切换；维护者随后直接将其标为 latest。本轮不做人机活体验收。`1.4.4` 维护工作已完成并打 tag，但从未作为面向用户的 GitHub Release 公开，内容已包含在 `1.5.0` 中；未公开的 `1.4.4` Draft Release 已删除；历史 tag 因仓库不可删除 ruleset 保持原 SHA。当前激活的下一版本是 `1.6.5`：在 `1.6.4` 之上纳入 Publish 工作流跳过过时 Android SDK `tools` 包的修复。`v1.6.0`–`v1.6.4` tag 不可移动；`v1.6.4` 指向 `17dbcf4e162abcc7e6380adeb9954eebd1cdfb91`，当前 `main` 已领先一提交，官方 Publish 无法从当前 `main` 复验该 Draft。从公开 `1.5.1` 到 `1.6.5` Draft 的 Windows x64 / Android ARM64 覆盖升级完整条尚未开始。`1.6.4` Draft 已由工作流 `34740686034` 签名且未 Publish。`1.6.3` Draft 已由工作流 `34738975651` 签名；维护者确认过 Android ARM64 上该版新功能可用。不增加新的缓存接口。
