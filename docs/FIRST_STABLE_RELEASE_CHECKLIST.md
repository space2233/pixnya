...
> 状态：首个稳定版与后续 `1.1.0`–`1.5.1` 已发布；当前维护范围是 `1.6.4`
> 当前升级基线：公开稳定版 `1.5.1`
...
- [ ] `1.6.4`：Windows x64 从公开 `1.5.1` 覆盖升级活体验收。尚未执行，不得预先填写 PASS、设备名或安装包 SHA-256。
- [ ] `1.6.4`：Android ARM64 从公开 `1.5.1` 覆盖升级，并覆盖标准、ECH、兼容三种连接模式。尚未执行。`1.6.3` 上维护者确认过新功能可用，不自动转记为本轮通过。
- [ ] Linux x64、Windows ARM64、Android ARM32：`1.6.4` 仅要求 CI 签名构建。
...
`1.5.1` 已是公开 latest stable；固定候选 `302e117e234aff16658c1a6f82c707c1697cd40e`、五平台签名、10 个附件已由 Draft 工作流 `33730303676` 生成。该 Release 曾作为 Pre-release 公开，官方 Publish 工作流只接受 Draft，无法再复验切换；维护者随后直接将其标为 latest。本轮不做人机活体验收。`1.4.4` 维护工作已完成并打 tag，但从未作为面向用户的 GitHub Release 公开，内容已包含在 `1.5.0` 中；未公开的 `1.4.4` Draft Release 已删除；历史 tag 因仓库不可删除 ruleset 保持原 SHA。当前激活的下一版本是 `1.6.4`：在 `1.6.3` 之上将音量键翻页选项限制为仅 Android 显示；`v1.6.0`、`v1.6.1`、`v1.6.2` 与 `v1.6.3` tag 不可移动。从公开 `1.5.1` 到 `1.6.4` Draft 的 Windows x64 / Android ARM64 覆盖升级完整条尚未开始。`1.6.3` Draft 已由工作流 `34738975651` 签名；维护者确认过 Android ARM64 上该版新功能可用。不增加新的缓存接口。
