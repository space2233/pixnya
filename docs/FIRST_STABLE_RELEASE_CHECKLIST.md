...
> 状态：首个稳定版与后续 `1.1.0`–`1.4.0` 已发布；当前维护候选为 `1.4.1`
> 当前升级基线：公开稳定版 `1.4.0`
...
- [x] 从固定提交 `fb40c46` 触发签名 Draft；GitHub Actions run `32035278025` attempt 2 完成五平台构建、签名与 10 附件汇总。
...
- [ ] Windows x64：从 `1.4.0` 升级到 `1.4.1` Draft，验证 NSIS、重启、登录状态、设置、数据库、离线文件和备份功能。
  - 已在 Windows 11 x64 验证静默安装、两版启动、覆盖安装、版本注册、离线 marker 和下载队列数据库 SHA-256 保留；升级前没有登录会话，登录状态、设置和界面备份流程仍待人工验证。
- [ ] Android ARM64：从 `1.4.0` 覆盖安装 `1.4.1` Draft，验证系统安装器、版本码、登录状态、设置、数据库、离线文件和备份功能。
- [ ] 标准、ECH、兼容三种连接模式在 Android ARM64 各完成一次低频登录、API 与图片加载冒烟测试。
- [x] Linux x64、Windows ARM64、Android ARM32 已完成签名 CI 构建；本轮不宣称已做活体验收。
...
`1.4.0` 已是公开 latest stable，生产签名材料与匿名更新源正常。`1.4.1` 发布前仍需固定候选提交、完成五平台签名 Draft，并补齐 Windows x64 与 Android ARM64 的升级和数据保留证据。
