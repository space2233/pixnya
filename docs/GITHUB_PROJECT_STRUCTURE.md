...
| Git tags | 如 `v1.5.1`，直接指向发布源码提交 | 将 Release 与不可变源码版本绑定 |
...
`PROJECT_PLAN.md` 保持本地忽略；当前仓库共有 395 个受跟踪文件。主要分组如下。
...
## 5. v1.5.1 Release 的 10 个附件

公开页面：<https://github.com/space2233/pixnya/releases/tag/v1.5.1>

### 用户安装包

| 文件 | 大小（约） | 用途 |
|---|---:|---|
| `PixNya_1.5.1_x64-setup.exe` | 6.53 MiB | Windows x64 NSIS 安装包 |
| `PixNya_1.5.1_arm64-setup.exe` | 5.73 MiB | Windows ARM64 原生应用安装包 |
| `PixNya_1.5.1_amd64.AppImage` | 84.00 MiB | Linux x64 AppImage |
| `pixnya-1.5.1-android-arm64-v8a.apk` | 30.93 MiB | Android ARM64 split APK，Android 10+ |
| `pixnya-1.5.1-android-armeabi-v7a.apk` | 23.77 MiB | Android ARM32 split APK，Android 10+ |
...
| `pixnya-1.5.1-verification.tar.gz` | 发布审计包：来源证明、SBOM、许可证归档、构建工具扫描结果和桌面签名 | 仅审计时需要 |
...
main 上的固定提交
  ...
  -> 独立重新下载和验签
  -> tag v1.5.1 + Latest Stable
```

`v1.5.1` tag 当前直接指向发布源码提交 `302e117e234aff16658c1a6f82c707c1697cd40e`。安装包不提交到 `main`，而是放在 Release；源码、构建规则和验证脚本则保存在 `main` 和 tag 中。本轮未再跑 provenance 恢复 finalizer；Draft 工作流提交与 tag 目标均为该源码 SHA。该 Release 曾作为 Pre-release 公开，后由维护者直接标为 latest；官方 Publish 工作流无法消费已发布的 Pre-release。
