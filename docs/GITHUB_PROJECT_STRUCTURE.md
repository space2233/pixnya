...
移除本地专用的 `PROJECT_PLAN.md` 后，原有源码基线为 358 个受跟踪文件；把本地与 GitHub 两份结构说明纳入后为 360 个。主要分组如下。
...
main 上的固定提交
  ...
  -> 独立重新下载和验签
  -> tag v1.3.0 + Latest Stable
```

`v1.3.0` tag 当前直接指向发布源码提交 `1f6db495da88af19f04d091381c0d8d65b869a09`。安装包不提交到 `main`，而是放在 Release；源码、构建规则和验证脚本则保存在 `main` 和 tag 中。
