...
> 状态：生产签名更新流程已投入使用；当前公开稳定版为 `1.5.1`，当前源码版本为 `1.6.4`
> 更新日期：2026-09-13
...
- [ ] `1.6.4`：沉浸小说阅读、分页裁剪、长文按页渲染、列表自动加载、阅读页页码避让，以及音量键翻页选项仅 Android 显示的签名 Draft；从公开 `1.5.1` 在 Windows x64 与 Android ARM64 做覆盖升级活体验收（Android 含三种连接模式）。活体验收尚未执行。`v1.6.0`–`v1.6.3` tag 不可移动。`1.6.3` Draft（`34738975651`）已签名且未 Publish；维护者确认过该版 Android 新功能。
