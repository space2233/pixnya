...
<AppShell title={m.settings_privacy()}><div class="page"><ReturnLink fallback="/settings" label={m.common_back()} /><h1>{m.settings_privacy()}</h1><section>
  ...
  <div class="row"><strong>{m.settings_diagnostic_log()}</strong><span>{logs?.entryCount ?? "—"}</span><button disabled={!logs||busy} onclick={exportLogs}>{m.settings_export_log()}</button><button disabled={!logs||busy} onclick={clearLogs}>{m.settings_clear_log()}</button></div>
...
<style>.page{width:min(720px,100%);box-sizing:border-box;margin:auto;padding:30px 24px 60px}h1{margin:24px 0;font-size:28px}section{overflow:hidden;border:1px solid var(--line);border-radius:18px;background:white}.row{display:flex;min-height:62px;align-items:center;gap:12px;padding:0 18px;border-bottom:1px solid var(--line);color:var(--text);text-decoration:none}.row:last-child{border:0}.row input,.row span,.row i{margin-left:auto}.row i{font-style:normal}.row button{padding:8px 12px;border:1px solid var(--line);border-radius:16px;background:white}.danger button{color:var(--danger)}.error{color:var(--danger)}.dialog-layer{position:fixed;z-index:1000;inset:0;display:grid;place-items:center;padding:20px}.scrim{position:absolute;inset:0;border:0;background:#0008}.dialog-layer>div{position:relative;width:min(420px,100%);box-sizing:border-box;padding:24px;border-radius:18px;background:white}.dialog-layer label span,.dialog-layer label input{display:block;width:100%;box-sizing:border-box}.dialog-layer input{margin-top:10px;padding:10px}.dialog-layer footer{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}.dialog-layer footer button{padding:10px 18px;border:1px solid var(--line);border-radius:20px;background:white}.dialog-layer footer .primary{background:var(--pixiv-blue);color:white}</style>
