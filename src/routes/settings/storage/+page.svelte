<script lang="ts">
  ...
  async function reload() {
    ...
    storage = nextStorage.status === "fulfilled" ? nextStorage.value : null;
  ...
  async function clearCacheNow() {
    await act(async () => { cache = await clearMediaCache(); storage = await getStorageStatus(); confirmCacheClear = false; });
  }
...
<AppShell title={m.settings_storage()}>
  <div class="page">
    ...
    <section>
      <div class="row"><strong>{m.settings_storage()}</strong><span>{storage ? bytes(storage.dataAvailableBytes) : m.settings_reading()}</span></div>
