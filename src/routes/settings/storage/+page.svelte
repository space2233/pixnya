...
<AppShell title={m.settings_storage()}>
  <div class="page">
    ...
    <section>
      ...
      <details class="cache-details"><summary>{m.settings_cache_details()}</summary><div><span>{m.settings_cache_thumbnails()}<strong>{cache ? bytes(cache.thumbnailBytes) : "—"}</strong></span><span>{m.settings_cache_previews()}<strong>{cache ? bytes(cache.previewBytes) : "—"}</strong></span><span>{m.settings_cache_originals()}<strong>{cache ? bytes(cache.originalBytes) : "—"}</strong></span></div></details>
