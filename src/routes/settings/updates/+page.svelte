...
<AppShell title={m.settings_updates()}><div class="page"><ReturnLink fallback="/settings" label={m.common_back()} /><h1>{m.settings_updates()}</h1><section>
...
<style>.page{width:min(720px,100%);box-sizing:border-box;margin:auto;padding:30px 24px 60px}h1{margin:24px 0;font-size:28px}section{overflow:hidden;border:1px solid var(--line);border-radius:18px;background:white}.row{display:flex;min-height:62px;align-items:center;justify-content:space-between;gap:18px;padding:0 18px;border-bottom:1px solid var(--line)}button{padding:8px 14px;border:1px solid #cde7f8;border-radius:18px;background:white;color:var(--brand)}input{accent-color:var(--brand)}.release{padding:18px}.release p{white-space:pre-wrap}.release footer{display:flex;gap:10px}.error{color:var(--danger)}</style>
