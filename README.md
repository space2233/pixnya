...
版本号使用 `主版本.功能版本.修复版本`：修复 bug 只增加第三位，新增功能增加第二位，只有不兼容的大改版才增加第一位。当前源码版本 `1.0.0` 是首个稳定版 Draft 候选；生产签名升级基线为 `0.29.0`。版本迁移只用于生成同密钥签名的目标安装包并完成三平台升级验收，不代表已经发布 stable。
