...
请前往 [PixNya v1.0.0 正式版](https://github.com/space2233/pixnya/releases/tag/v1.0.0)，按设备选择一个安装包：

- Windows 64 位：`PixNya_1.0.0_x64-setup.exe`
- Linux 64 位：`PixNya_1.0.0_amd64.AppImage`
- Android 10 及以上、ARM64：`pixnya-1.0.0-android-arm64-v8a.apk`

Release 中的 JSON 和 minisign 文件仅供应用自动更新使用，普通用户无需下载。
...
版本号使用 `主版本.功能版本.修复版本`：修复 bug 只增加第三位，新增功能增加第二位，只有不兼容的大改版才增加第一位。当前稳定版为 `1.0.0`；生产签名升级基线为 `0.29.0`。
